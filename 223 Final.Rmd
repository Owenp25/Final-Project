...
heart-health related predictor variables. We’ve looked at classifying chest type as well, but
we are unsure if we will keep that in the final version. The data used in this study consists of
5 independent sub-datasets of heart health related data. The main response variables
...
because the first to PC’s only account for around 55%, and ⅘ of the PC’s would get us to
...
value for cp was determined to be 0.0182927:

xerror < min(xerror) + min(xstd)
0.182927 < 0.36341 + 0.027249

The output of the pruned tree is shown above This tree returned an accuracy score of
83.55%. ST_Slope was the first predictor variable considered – those with an ST_Slope
Down, Flat were classified as affected by heart disease. For those with an ST_Slope Up,
patients with a cholesterol greater than or equal to 43 were classified as unaffected by heart
disease, and patients with a cholesterol lower than 43 were classified as affected by heart
disease.
...
```
The ideal value for cp for a decision tree for chest pain was determined to be 0.04502370:

xerror < min(xerror) + min(xstd)
0.04502370 < 0.85071 + 0.035036

The output of the pruned tree is shown above This tree returned an accuracy score of
61.22%. Heart Disease was the first factor considered in the tree with those affected
classified as having ASY chest pain. For those unaffected, those with an Old Peak greater
than or equal to 0.05 were classified as having NAP chest pain, and those with an Old Peak
less than 0.05 were classified as having ATA chest pain. TA chest pain was not classified at
all in this tree.
...
accuracy of about 83.55% in predicting heart disease status. This means that given data of a
new patient in the same format as used in the model, we have around an 83.55% chance of
...
model. KNN was very marginally better than our decision tree with an accuracy of 61.98%
versus 61.22%. Because this difference is so small and decision trees are more easily
